...
# VERSION 3: implementing all heuristics incrementally
...
def conflict_heuristic(board):
    ...
    # do the rows first
    for row in range(0, SIZE):
        heuristic += conflict(helper_row(row, board))
    # columns next
    for col in range(0, SIZE):
        heuristic += conflict(helper_col(col, board))
    return 2 * heuristic
...
def get_children(board):
    ...
    if b > 0:
        ...
        if col == b - 1 or col == b:
            net_change += 2 * conflict(helper_col(col, child)) - 2 * conflict(helper_col(col, board))
    ...
    if a < size - 1:
        ...
        child = board[0 : min(ind, ind2)] + board[max(ind, ind2)] + board[min(ind, ind2) + 1 : max(ind, ind2)] + board[min(ind, ind2)] + board[max(ind, ind2) + 1 : ]
        if row == a + 1 or row == a:
            net_change += 2 * conflict(helper_row(row, child)) - 2 * conflict(helper_row(row, board))
        children.append((child, net_change))
