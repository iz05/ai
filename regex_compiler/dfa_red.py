...
def DFA(nfa):
    ...
    next_node = len(nfa)
    hybrids_tracker = {}
    hybrids_tracker_2 = {} # [node] = {hybrid nodes it's a part of}
    while True:
        h = get_first_hybrid(nfa, alphabet)
        if h == -1:
            return (nfa, final_nodes, alphabet)

        # find all hybrid nodes first
        hybrids = {}
        for node in nfa:
            for char in alphabet:
                if len(nfa[node][char]) > 1:
                    s = frozenset(nfa[node][char])
                    if s not in hybrids:
                        hybrids[s] = (next_node, )
                        hybrids_tracker[s] = next_node
                        next_node += 1
                    if node not in hybrids_tracker_2:
                        hybrids_tracker_2[node] = set()
                    hybrids_tracker_2[node].add(hybrids_tracker[s])
        ...
        for node in nfa:
            for char in alphabet:
                if len(nfa[node][char]) > 1:
                    s = frozenset(nfa[node][char])
                    hybrid_node = hybrids[s][0]
                    nfa[node][char] = (hybrid_node, )
        
        # find all reachables
        for s in hybrids:
            for c in alphabet:
                hybrids[s] = hybrids[s] + (reachable_with_char(c, s, nfa, final_nodes), )
        
        # connect those reachables, important: include the hybrid node logic
        for s in hybrids:
            hybrid_node = hybrids[s][0]
            nfa[hybrid_node] = {}
            index = 1
            for char in alphabet:
                r, final = hybrids[s][index]
                delete = set()
                for node2 in r:
                    for node3 in r:
                        if node2 in hybrids_tracker_2 and node3 in hybrids_tracker_2[node2]:
                            delete.add(node2)
                r = set(r)
                for node2 in delete:
                    r.remove(node2)
                r = frozenset(r)
                # print("Hybrid node %s: connects to %s with char %s." % (hybrid_node, str(r), char))
        ...
        # display_dict_nfa(nfa, alphabet)   
        # print("Final nodes: " + str(final_nodes))
        # print(hybrids_tracker) # @debug
        # print() # @debug
        # input() # @debug  
    ...
    #     if h == -1:
    #         return (nfa, final_nodes, alphabet)
    #     data = hybrid(nfa, alphabet, h)
    #     hybrid_node = next_node
    #     next_node += 1
    #     nfa[hybrid_node] = {}
    #     for char in alphabet:
    #         nfa[hybrid_node][char] = ()
    #     for tup in data[0]:
    #         node, char = tup
    #         nfa[node][char] = (hybrid_node, )
    #     for node in h: # hybrid_node has to inherit all properties of node
    #         if node in final_nodes:
    #             final_nodes.add(hybrid_node)
    #         for char in alphabet:
    #             for node2 in nfa[node][char]:
    #                 if node2 not in nfa[hybrid_node][char]:
    #                     nfa[hybrid_node][char] = nfa[hybrid_node][char] + (node2, )
                    # elif node2 not in nfa[hybrid_node][char] and node2 in h and hybrid_node not in nfa[hybrid_node][char]:
                    #     nfa[hybrid_node][char] = nfa[hybrid_node][char] + (hybrid_node, )
        # display_dict_nfa(nfa, alphabet)
        # print("Final nodes: " + str(final_nodes))
        # print()
        # input()
