...
def NFAepsilon(regex, alphabet):
    ...
    while(True):
        ...
        if data == -1:
            draw_graph(str(GRAPH_COUNT), graph, final_nodes, True)
            break
        else:
            draw_graph(str(GRAPH_COUNT), graph, final_nodes, False)
            key, index = data
            start = key
...
def NFA(nfa):
    ...
    for node in nfa:
        ...
        draw_nfa(str(GRAPH_COUNT), nfa2, final_nodes, alphabet, False)
    draw_nfa(str(GRAPH_COUNT), nfa2, final_nodes, alphabet, True)
...
def make_hybrid(nfa, nfa_old, final_nodes, alphabet, set_of_nodes, hybrids_tracker):
    hybrid_node = len(nfa)
    hybrids_tracker[frozenset(set_of_nodes)] = hybrid_node
...
def DFA(nfa):
    ...
    while True:
        ...
        if s == -1:
            draw_nfa(str(GRAPH_COUNT), nfa, final_nodes, alphabet, True)
            return (nfa, final_nodes, alphabet)
        draw_nfa(str(GRAPH_COUNT), nfa, final_nodes, alphabet, False)
...
def optimize(dfa):
    ...
    draw_dfa(str(GRAPH_COUNT), dfa, final_nodes, alphabet, False)
    ...
    while(True):
        ...
        if pair == -1:
            draw_dfa(str(GRAPH_COUNT), dfa, final_nodes, alphabet, True)
...
def draw_graph(graph_name, graph, final_nodes, bool):
    global GRAPH_COUNT
    GRAPH_COUNT += 1
    gra = graphviz.Digraph()
    if bool:
        gra.attr('node', color = 'red')
    gra.attr('node', shape = 'doublecircle')
    ...
    for node in graph:
        if node not in final_nodes:
            gra.node(str(node))
    for node in graph:
        for node2, regex in graph[node]:
            gra.edge(str(node), str(node2), label = regex)
    gra.render("graphs/" + graph_name + ".gv", view = False)

def draw_nfa(graph_name, graph, final_nodes, alphabet, bool):
    global GRAPH_COUNT
    GRAPH_COUNT += 1
    gra = graphviz.Digraph()
    if bool:
    ...
    for node in graph:
        for char in alphabet:
            for node2 in graph[node][char]:
                gra.edge(str(node), str(node2), label = char)
    gra.render("graphs/" + graph_name + ".gv", view = False)

def draw_dfa(graph_name, graph, final_nodes, alphabet, bool):
    global GRAPH_COUNT
    GRAPH_COUNT += 1
    gra = graphviz.Digraph()
    if bool:
    ...
    for node in graph:
        for char in alphabet:
            if char in graph[node]:
                gra.edge(str(node), str(graph[node][char]), label = char)
    gra.render("graphs/" + graph_name + ".gv", view = False)

def make_pdf():
    merger = PdfFileMerger()
    for i in range(0, GRAPH_COUNT):
        merger.append("graphs/" + str(i) + ".gv.pdf")
    # merger.write("debug.pdf") # @debug
    merger.write("conversion.pdf")
    merger.close()
