...
print("Finished generating all possible combinations for rows and columns in %s seconds." % (end - start))
# with open(sys.argv[1]) as f:
#     i = 0
#     total_start = time.perf_counter()
#     for line in [line.strip() for line in f][:30]:
#         start = time.perf_counter()
...
start = time.perf_counter()
length = a_star(SIZE, ".JAFNHBIOMKGLDEC")
end = time.perf_counter()
print((end - start))