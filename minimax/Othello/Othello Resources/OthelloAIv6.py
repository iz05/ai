...
def find_next_move(board, player, depth):
    ...
    return nega_scout(board, depth, -100000000, 100000000, player)[1]
...
def nega_scout(board, depth, alpha, beta, token):
    switch = {
        "o" : "x",
        "x" : "o"
    }
    ...
    for move in moves:
        ...
        if max_move != -1:
            sc = -1 * nega_scout(temp_board, depth - 1, -1 * alpha - 1, -1 * alpha, switch[token])[0]
            ...
            #     else:
            #         temp_board = make_move(board, token, move)
            #         max_score = -1 * nega_scout(temp_board, depth - 1, -1 * beta, -1 * sc, switch[token])[0]
            if sc > alpha and sc < beta:
                temp_board = make_move(board, token, move)
                sc = -1 * nega_scout(temp_board, depth - 1, -1 * beta, -1  * sc, switch[token])[0]
        else:
            sc = -1 * nega_scout(temp_board, depth - 1, -1 * beta, -1 * alpha, switch[token])[0]
...
def score(board): # returns the score for a board and token
    ...
    heuristic = mobility * (1 - board_fill) + board_fill * weighted_count
    return heuristic
...
# with open("boards_timing_my_results_negascout.csv", "w") as g:
#     for l in results:
#         g.write(", ".join(l) + "\n")