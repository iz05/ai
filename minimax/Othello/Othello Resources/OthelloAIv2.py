...
# current version on TJ Othello Server, replaced by OthelloAIServer on 11/9/21
...
def score(board): # returns the score for a board and token
    ...
    if board_fill == 1: # board is filled
        return 100000 * (x_count - o_count)

    mobility = len(possible_moves(board, "x")) - len(possible_moves(board, "o")) # calculating mobility
    # heuristic =  board_fill * board_fill * 5 * (x_count - o_count) + board_fill * (1 - board_fill)  * weighted_count + (1 - board_fill) * 5 * mobility # weighting mobility and difference in count for a heuristic
    ...
    heuristic = mobility + 5 * diff_corner
