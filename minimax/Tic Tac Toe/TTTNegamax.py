...
def max_move(board):
    ...
    if type(data) == int:
        ...
        return data
    results = []
    index = -1 # index and cur are wrong
    cur = 0
    for b in data[1]:
        val = min_step(b)
...
def negamax(board, token):
    ...
    if token == "X":
        next_token = "O"
    data = next_moves(board)
    if type(data) == int: # if data is an integer, 1 represents X winning, 0 represents tie, -1 represents O winning
        if data == 0:
            return 0
        elif data == 1 and token == "X" or data == -1 and token == "O": # X wins or O wins
            return 1
        elif data == 1 and token == "O" or data == -1 and token == "X": # X loses or O loses
            return -1
        else:
            print("Error in negamax method")
    results = []
    for b in data[1]:
        results.append(-1 * negamax(b, next_token))
    return max(results)

def negamove(board, token):
    next_token = "X"
    if token == "X":
        next_token = "O"
    data = next_moves(board)
    if type(data) == int:
        ...
        if data == 0:
            ...
            print("Error in negamove method")
    results = []
    index = 0
    cur = 0
    for b in data[1]:
        val = negamax(b, next_token)
        ...
        # finds the move that guarantees the best outcome
        results.append(val)
        if val < results[index]:
            index = cur
        cur += 1
...
def play_game(board):
    ...
    while(True):
        ...
        if cur_turn == "player": # player's turn
            ...
            index = -1
            index = negamove(board, computer_token)
            print()
