...
# version 1: Experimenting with different heuristics for blocks
...
def get_spaces(state):
    ...
    spaces = list(spaces)
    # spaces.sort(key = lambda s : heuristic(s, state))
    # spaces.sort(key = lambda s : 700 * heuristic(s, state) - (s[0] + 1) * (width - s[0]) * (s[1] + 1) * (height - s[1]))
    # spaces.sort(key = lambda s : -1 * (s[0] + 1) * (width - s[0]) * (s[1] + 1) * (height - s[1]))
    spaces.sort(key = lambda s : -1 * heuristic2(s, state))
    return spaces # returns all squares with "."

def heuristic(space, state):
    count = 1
    for sp in state[1]:
        if sp[0] == space[0] or sp[1] == space[1]:
            count += 1
        if abs(sp[0] - space[0]) + abs(sp[1] - space[1]) <= 1:
            count += 50
    return count

def heuristic2(space, state): # product of length of words
...
def generate_data(state): # fills horizontal, vertical
    global horizontal, vertical, word_bank
    puzzle, block_squares, blank_squares = state
...
def get_options(state, blank):
    h_word = get_h_word(state[0], blank)
    ...
    v_word = get_v_word(state[0], blank)
    ...
    return letters_h.intersection(letters_v)
