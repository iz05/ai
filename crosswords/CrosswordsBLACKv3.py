...
# version 3: word, letter dual recursion
...
def generate_data(state): # fills horizontal, vertical
    global horizontal, vertical, word_bank, h_neighbors, v_neighbors, words_by_size, templates_in_options, options_global, words_2, num_of_words, words_h, words_v
    puzzle, block_squares, blank_squares = state
    ...
    for space in blank_squares:
        ...
        while (row, col + i) in blank_squares:
            ...
            words_h[space].append((row, col + i))
            i = i - 1
        words_h[space].reverse()
        words_h[space].append(space)
        pos = -1 * i - 1
        i = 1
        while (row, col + i) in blank_squares:
            h_neighbors[space].add((row, col + i))
        ...
        while (row + i, col) in blank_squares:
            ...
            words_v[space].append((row + i, col))
            i = i - 1
        words_v[space].reverse()
        words_v[space].append(space)
        pos = -1 * i - 1
        i = 1
        while (row + i, col) in blank_squares:
            v_neighbors[space].add((row + i, col))
    ...
    # fills in words_2 ([(blank, orientation)] -> number (symbolizing the word))
    i = 0
    for blank in blank_squares: # do horizontal words first
        if (blank, "h") not in words_2:
            words_2[(blank, "h")] = i
            for neighbor in h_neighbors[blank]:
                words_2[(neighbor, "h")] = i
            i += 1
    for blank in blank_squares: # do vertical words next
        if (blank, "v") not in words_2:
            words_2[(blank, "v")] = i
    ...
    for blank in blank_squares:
        i = words_2[(blank, "h")]
        if i not in word_options:
            template = get_h_word(puzzle, blank)
            possible_words = set()
            for word in words_by_size[len(template)]:
                if fit(template, word):
                    possible_words.add(word)
            word_options[i] = frozenset(possible_words)
            
    for blank in blank_squares:
        i = words_2[(blank, "v")]
        if i not in word_options:
            template = get_v_word(puzzle, blank)
            possible_words = set()
            for word in words_by_size[len(template)]:
                if fit(template, word):
                    possible_words.add(word)
    ...
    options = {}
    for blank in blank_squares:
        options[blank] = get_options(puzzle, word_options, blank)
        if len(options[blank]) == 1:
            del options[blank]

    return (puzzle, positions, words_used, options, word_options)
...
def get_options(puzzle, word_options, blank):
    h_word = get_h_word(puzzle, blank)
    if h_word not in word_bank:
        possible_words_h = set()
        for word in word_options[words_2[(blank, "h")]]:
            if fit(h_word, word):
    ...
    if v_word not in word_bank:
        ...
        for word in word_options[words_2[(blank, "v")]]:
            if fit(v_word, word):
...
def get_options_2(word_options, blank):
    position_h = horizontal[blank][1]
    possible_letters_h = set()
    for word in word_options[words_2[(blank, "h")]]:
        possible_letters_h.add(word[position_h])
    position_v = vertical[blank][1]
    possible_letters_v = set()
    for word in word_options[words_2[(blank, "v")]]:
        possible_letters_v.add(word[position_v])
    return frozenset(possible_letters_h.intersection(possible_letters_v))

def place_and_update_fill(state, blank, letter):
    puzzle, positions, words_used, options, word_options = state
    row, col = blank
    ...
    state = update_words(state, blank)
    return state

# dual recursion methods
def update_letters(state, blank):
    puzzle, positions, words_used, options, word_options = state
    if blank in options:
        letters = get_options_2(word_options, blank)
        if len(letters) == 0:
            return None
        if len(letters) < len(options[blank]):
            options[blank] = letters
            return update_words(state, blank)
    return state

def h_works(state, word, blank):
    puzzle, positions, words_used, options, word_options = state 
    for i in range(0, len(word)):
        cur_blank = words_h[blank][i]
        if cur_blank in options:
            if word[i] not in options[cur_blank]:
                return False
        else:
            row, col = cur_blank
...
def v_works(state, word, blank):
    puzzle, positions, words_used, options, word_options = state 
    for i in range(0, len(word)):
        cur_blank = words_v[blank][i]
        if cur_blank in options:
            if word[i] not in options[cur_blank]:
                return False
        else:
...
def update_words(state, blank):
    puzzle, positions, words_used, options, word_options = state
    new_h = set()
    flag = False
    h_word = get_h_word(puzzle, blank)
    for word in word_options[words_2[(blank, "h")]]:
        if h_works(state, word, blank):
            new_h.add(word)
        else:
            flag = True
    if len(new_h) == 0:
        return None
    word_options[words_2[(blank, "h")]] = frozenset(new_h)
    # if flag:
    #     for neighbor in h_neighbors[blank]:
    #         state = update_letters(state, neighbor)
    #         if state is None:
    #             return None
    ...
    v_word = get_v_word(puzzle, blank)
    for word in word_options[words_2[(blank, "v")]]:
        if v_works(state, word, blank):
            new_v.add(word)
        else:
            flag = True
    if len(new_v) == 0:
        return None
    word_options[words_2[(blank, "v")]] = frozenset(new_v)
...
def backtrack_fill(state):
    ...
    if state is not None:
        ...
        if var is not None:
            ...
            letters = sorted(letters, key = lambda letter : -1 * heuristic_fill(blank, letter, puzzle))
            for letter in letters:
                new_state = (puzzle.copy(), positions.copy(), words_used.copy(), options.copy(), word_options.copy())
...
def heuristic_fill(blank, letter, puzzle):
    h_word = get_h_word(puzzle, blank)
    length, position = horizontal[blank]
    h_word = h_word[ : position] + letter + h_word[position + 1 :]
    h_count = 0
    if h_word not in word_bank:
        ...
        h_count = len(word_bank[h_word])

    v_word = get_h_word(puzzle, blank)
    length, position = vertical[blank]
    v_word = v_word[ : position] + letter + v_word[position + 1 :]
    v_count = 0
    if v_word not in word_bank:
...
if state is not None:
    ...
    print("None")
print("Backtracked %s times." % backtrack_count)
