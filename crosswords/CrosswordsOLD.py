...
def process_input(): # reads in data
    ...
    for i in range(2, len(args)):
        ...
        col = int(num)
        # if dim[1][-1].isalpha():
        #     m = dim[1].find(next(filter(str.isalpha, dim[1])))
        #     col = int(dim[1][ : m])
        #     word = dim[1][m : ]
        word = temp[len(num) : ]
        for char in word:
...
def backtrack(state): # returns a valid blocking of a puzzle
    ...
    if len(block_squares) == blocks:
        ...
        if valid_blocking(state):
            ...
            return None
    if len(blank_squares) == blanks:
        print(len(blank_squares))
        for i in range(0, height):
            for j in range(0, width):
                if puzzle[i][j] == ".":
                    state = place_and_update("#", state, i, j)
    ...
    for space in get_spaces(state):
        ...
        new_puzzle = backtrack(new_state)
...
def build(): # returns the built puzzle
    ...
    # print_board(state[0])
    # print("Done")
    # print(valid_blocking(state))
    puzzle = backtrack(state)
    if puzzle is None:
