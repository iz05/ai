...
def fit_2(template, word, orientation, blank, options):
    row, col = blank
    if orientation == "h":
        position = horizontal[blank][1]
        cur_blank = (row, col - position)
        for i in range(0, len(word)):
            if cur_blank in options:
                ...
                if word[i] != template[i]:
                    return False
            cur_blank = (cur_blank[0], cur_blank[1] + 1)
        return True
    if orientation == "v":
        position = vertical[blank][1]
        cur_blank = (row - position, col)
        for i in range(0, len(word)):
            if cur_blank in options:
                if word[i] not in options[cur_blank]:
                    return False
                ...
                if word[i] != template[i]:
                    return False
            cur_blank = (cur_blank[0] + 1, cur_blank[1])
        return True

def get_options_2(puzzle, options, blank):
    h_word = get_h_word(puzzle, blank)
    possible_words_h = set()
    for word in words_by_size[len(h_word)]:
        if fit_2(h_word, word, "h", blank, options):
            possible_words_h.add(word)
    letters_h = set()
    position_h = horizontal[blank][1]
    ...
    possible_words_v = set()
    for word in words_by_size[len(v_word)]:
        if fit_2(v_word, word, "v", blank, options):
            possible_words_v.add(word)
    letters_v = set()
    position_v = vertical[blank][1]
    for word in possible_words_v:
        letters_v.add(word[position_v])
    return frozenset(letters_h.intersection(letters_v))
...
def place_and_update_fill(state, blank, letter):
    ...
    state = update_options(state, blank)
    return state

def update_options(state, blank):
    puzzle, positions, words_used, options = state
    for h in h_neighbors[blank]:
        if h in options:
            options_h = get_options_2(puzzle, options, h)
            if len(options_h) == 0:
                return None
            elif len(options_h) == 1:
                state = forward_looking_fill(state)
                if state is None:
                    return None
            elif len(options_h) < len(options[h]):
                options[h] = options_h
                state = update_options(state, h)
                if state is None:
                    return None
    for v in v_neighbors[blank]:
        if v in options:
            options_v = get_options_2(puzzle, options, v)
            if len(options_v) == 0:
                return None
            elif len(options_v) == 1:
                state = forward_looking_fill(state)
                if state is None:
                    return None
            elif len(options_v) < len(options[v]):
                options[v] = options_v
                state = update_options(state, v)
                if state is None:
                    return None
    return state
