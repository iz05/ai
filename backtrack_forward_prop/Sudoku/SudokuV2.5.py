...
# board will be stored as (list of strings, solved indices queue), each string stores all possibilities of characters
...
def get_next_variable(state_tuple):
    state = state_tuple[0]
    smallest_length = N + 1
    var = -1
    for i in range(0, N * N):
...
def forward_looking(state_tuple):
    state, solved = state_tuple
    new_state = state.copy()
    new_solved = solved.copy()
    temp_solved = solved.copy()

    while len(temp_solved) != 0:
        index = temp_solved.popleft()
        for neighbor in constraints[index]:
            if new_state[index] in new_state[neighbor]:
                ...
                if len(new_state[neighbor]) == 0:
                    ...
                    temp_solved.append(neighbor)
                    new_solved.append(neighbor)
    
    return (new_state, new_solved)
...
def backtrack(state):
    ...
    for value in get_sorted_values(index, state):
        state_1 = state[1].copy()
        state_1.append(index)
        new_state = (state[0].copy(), state_1)
        new_state[0][index] = value
