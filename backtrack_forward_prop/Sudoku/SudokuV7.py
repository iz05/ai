...
def forward_looking(new_state):
    ...
    if change == True:
        constraint_propagation(new_state)
...
def place(state, index, value):
    ...
    for neighbor in constraints[index]:
        if value in state[neighbor]:
            ind = state[neighbor].index(value)
            state[neighbor] = state[neighbor][0 : ind] + state[neighbor][ind + 1 : ]
            if len(state[neighbor]) == 0:
                state = None
                return
            remove(state, neighbor, value)
            if state is None:
                return
...
def remove(state, index, value):
    ...
    for set in constraint_sets_2[index]:
        # check if there is only one square with a possible value
        count = 0
        ind = -1
        for item in set:
            ...
            if val1 != value:
                ...
                for item in set:
                    if val1 in state[item] and value in state[item]:
                        indices.append(item)
                    elif val1 in state[item] and value not in state[item]:
                        return
                    elif val1 not in state[item] and value in state[item]:
                        return
                if len(indices) == 2:
