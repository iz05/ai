...
def place(state, index, value):
    ...
    for i in range(0, len(state[index])):
        char = state[index][i]
        if char != value:
            ind = new_state[index].find(char)
            new_state[index] = new_state[index][0 : ind] + new_state[index][ind + 1 : ]
            new_state = remove(new_state, index, char)
    ...
    for neighbor in constraints[index]:
        if value in new_state[neighbor]:
            ind = new_state[neighbor].index(value)
            new_state[neighbor] = new_state[neighbor][0 : ind] + new_state[neighbor][ind + 1 : ]
            if len(new_state[neighbor]) == 0:
                return None
            new_state  = remove(new_state, neighbor, value)
            if new_state is None:
                return new_state
