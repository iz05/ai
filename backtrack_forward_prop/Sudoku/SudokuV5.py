    ...
    while len(one_solution_indices) != 0:
        ...
        for neighbor in constraints[index]:
            if new_state[index] in new_state[neighbor]:
                ...
                remove_item(new_state, neighbor, new_state[index])
...
def remove(state, index, value):
    ...
    n = len(state[index])
    if n != 1: # delete this if statement, only slows down code
    ...
    # for c_set in constraint_sets_2[index]:
    #     values_in_other_squares = set()
    #     indices_containing_value = []
    #     for item in c_set:
    #         if value in state[item]:
    #             indices_containing_value.append(item)
    #         else:
    #             for val in state[item]:
    #                 values_in_other_squares.add(val)
    #     x = len(indices_containing_value)
    #     values = set()
    #     for val in CHAR_STRING:
    #         if val not in values_in_other_squares:
    #             add = True
    #             for ind in indices_containing_value:
    #                 if val not in state[ind]:
    #                     add = False
    #                     break
    #             if add:
    #                 values.add(val)
    ...
    #                     if remove_item(state, ind, val):
    #                         state = remove(state, ind, val)
    #                         if state is None:
    #                             return None
